---
title: "Control vs. Infected groupings - Gene"
author: "Jared J. Homola"
date: "6/8/2020"
output: html_document
---
This is an analysis of differential expression at the gene level between control and infected groupings of blue-winged teals.
...
table(rowSums(dge.bursa$counts==0) == length(cnt.bursa[1,])) #Any genes w/o expression?
...
#### Retain only gene expressed at >0.5 CPM in at least 25% of individuals
...
deGeneCounts.ileum <- lcpm.ileum %>% 
  ...
  rownames_to_column("gene") %>%
  filter(gene %in% dt.tib.ileum$gene) %>% 
  column_to_rownames("gene")

newNames1 <- colnames(deGeneCounts.ileum) %>%
  as_tibble() %>%
  separate(value, into = c(NA, NA, "sample", NA, "SSAVG")) %>%
  #mutate(group = fct_relevel(group, "Ctl", "I01", "I03", "I05", "I14")) %>%
  #arrange(group) %>%
  unite("sample", 2:1, sep = "_")

colnames(deGeneCounts.ileum) <- newNames1$sample
...
hc <- hclust(as.dist(1-cor(t(deGeneCounts.ileum))))
...
deGeneCounts.bursa <- lcpm.bursa %>% 
  as_tibble(rownames = NA) %>% 
  rownames_to_column("gene") %>%
  filter(gene %in% dt.tib.bursa$gene) %>% 
  column_to_rownames("gene")
...
hc <- hclust(as.dist(1-cor(t(deGeneCounts.bursa ))))
