...
pathway.dat <- annot %>%
  select(
    ...
    sprot_Top_BLASTX_hit,
    Kegg) %>%
  rename(trans = transcript_id) %>%
  separate(sprot_Top_BLASTX_hit, into = c("sprot1", NA, NA, NA, NA, "sprot2", NA), "\\^") %>%
  separate(sprot2, sep = "=", into = c(NA, "SwissProt_GeneFunction")) %>%
  separate(sprot1, sep = "_", into = c("SwissProt_GeneName", NA)) %>%
  separate(Kegg, into = c("KEGG_ID", "KO_ID"), sep = "\\`") %>%
  distinct(trans, SwissProt_GeneName, KEGG_ID, .keep_all = TRUE) %>%
  select(trans, SwissProt_GeneName, SwissProt_GeneFunction, KEGG_ID, KO_ID) %>%
  distinct(trans, .keep_all = TRUE) %>%
  filter(grepl("KO:", KO_ID)) %>%
...
targets <- annot %>%
  ...
  inner_join(dt.tib) %>% 
  separate(Kegg, into = c("KEGG_ID", "KO_ID"), sep = "\\`") %>% 
  distinct(transcript, KEGG_ID, .keep_all = TRUE) %>% 
  na.omit(KO_ID) %>% 
  mutate(KO_ID = str_remove(KO_ID, "KO:"))

x <- enrichKEGG(targets$KO_ID,
                organism="ko",
                keyType="kegg",
                pvalueCutoff = 0.1, 
                pAdjustMethod = "BH",
